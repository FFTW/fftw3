...
let realeven2_gen n =
  ...
  let code  = no_twiddle_gen_expr (2*n) Symmetry.realeven2_input_sym FORWARD in
  let code' = vect_optimize varinfo_realeven n code in
  ...
  let in_unparser'  = ([], strided_real_unparser (input,istride4p))
  and out_unparser' = ([], strided_real_unparser (output,ostride4p)) in
  let unparser = make_asm_unparser_notwiddle in_unparser' out_unparser' in
    (n, FORWARD, REALEVEN2, initcode', vsimdinstrsToK7vinstrs unparser code')
