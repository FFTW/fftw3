...
/* TODO: this file is still a little messy */
...
/* transform a single input array with the help of a buffer */
static void apply(plan *ego_, R *ri, R *ii, R *ro, R *io)
{
     ...
     /* transform to buffer then copy back: */
     cld->apply(ego->cld, ri, ii, bufs, bufs + 1);
     cldcpy->apply(ego->cldcpy, bufs, bufs + 1, ro, io);
}
...
static void destroy(plan *ego_)
{
     P *ego = (P *) ego_;
     if (ego->bufs) fftw_free(ego->bufs);
     if (ego->cld_rest) fftw_plan_destroy(ego->cld_rest);
...
static int applicable(const problem *p_, const S *ego)
{
     if (DFTP(p_)) {
	  ...
	  /* 
	   * should decide whether to be BAD or UGLY when
	   * problem is too big.
	   */
	  if (p->vecsz.rnk > 1 || p->sz.rnk != 1 || toobig(d[0].n, ego))
	       return 0;

	  /*
	   * In principle, the buffered transforms might be useful when
	   * working out of place.  However, in order to prevent infinite
	   * loops in the planner, we require that the output stride of the
	   * buffered transforms be less than os. 
	   */
	  if (p->ri != p->ro)
	       return (d[0].os > 2);
...
static plan *mkplan(const solver *ego_, const problem *p_, planner *plnr)
{
     ...
     cldp = 
	  fftw_mkproblem_dft_d(
		...
		fftw_mktensor_2d(nbuf, bufdist * 2, p->vecsz.dims[0].os, 
				 n, 2, p->sz.dims[0].os)),
     ...
     /* If necessary, plan the leftover transforms (cld_rest): */
     if (vl % nbuf != 0) {
	  iodim *d = p->vecsz.dims;
	  cldp = fftw_mkproblem_dft_d(
	       fftw_tensor_copy(p->sz),
	       fftw_mktensor_1d(vl % nbuf, d->is, d->os),
	       p->ri, p->ii, p->ro, p->io);
     ...
     pln = MKPLAN_DFT(P, &padt, vl == 1 ? apply : apply_vec);
     pln->cld = cld;
     pln->cldcpy = cldcpy;
