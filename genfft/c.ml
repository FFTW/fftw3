...
let extract_constants f =
  let constlist = 
    flatten (map expr_to_constants (fcn_to_expr_list f))
  in let unique_constants =
    fold_left add_float_key_value [] constlist
  in let use_const () = foldr_string_concat
      (map (fun n ->
	"static const " ^ realtype ^ " " ^
	(Number.to_konst n) ^ " = " ^ 
	"K(" ^ (Number.to_string n) ^ ");\n")
	 unique_constants)
  and use_compact () = foldr_string_concat
      (map (fun n ->
	"DK(" ^ (Number.to_konst n) ^ ", " ^ (Number.to_string n) ^ ");\n")
	 unique_constants)
  and use_table () = 
    match length unique_constants with
      0 -> ""
    | l ->
	let t = "KT" in
	(Printf.sprintf "static const %s %s[%d] = {\n" realtype t l) ^
	(foldr_string_concat
	   (map2 (fun n i ->
	     (Printf.sprintf "K(%s),\n" (Number.to_string n)) ^
	     (Printf.sprintf "#define %s (%s[%d])\n" (Number.to_konst n) t i))
	      unique_constants (Util.iota l)))
	^ "};"

  in 
  if !Magic.konstant_table then 
    use_table () ^ "\n\n"
  else if !Magic.compact then 
    use_compact () ^ "\n\n" 
  else
    use_const () ^ "\n\n"
